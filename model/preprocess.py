...
class DataAugmentation:
    ...
    def process_data(self):
        ...
        postal_codes = self.postal_codes_belgium.assign(
            ...
            region=self.postal_codes_belgium["Région name (French)"],
            province=self.postal_codes_belgium["Province name (French)"].combine_first(
                self.postal_codes_belgium["Province name (Dutch)"]
            ),
        ...
        selected_columns = ["municipality", "region", "province", "prosperity_index"]
        new_data = data[selected_columns]
        new_data = new_data.drop_duplicates(subset=["municipality"])
        new_data.loc[
            (new_data["province"].isna())
            & (new_data["region"] == "région de bruxelles-capitale"),
            "province",
        ] = "région de bruxelles-capitale"

        new_data["province"] = (
            new_data["province"].str.replace(r"\s*\(.*?\)", "", regex=True).str.strip()
        )
        new_data = new_data.applymap(
            lambda x: x.title().strip() if isinstance(x, str) else x
        )
        new_data = new_data.dropna(subset=["municipality"])
        new_data.to_csv("features_data.csv", index=False)
        print(f"Features have been saved to features_data.csv")

        # Apply title case and remove extra spaces for 'province', 'region', and 'municipality' columns
        data["province"] = (
            data["province"].str.replace(r"\s*\(.*?\)", "", regex=True).str.strip()
        )
        columns_to_format = ["region", "province", "municipality"]
        data[columns_to_format] = data[columns_to_format].applymap(
            lambda x: x.title().strip() if isinstance(x, str) else x
        )

        # Drop columns
        data = data.drop(columns="nis")
...
class DataImputation:
    ...
    def impute_province(self):
        """
        Impute missing values in the 'province' column:
        - If the region is 'région de bruxelles-capitale', set the province to the same as the region.
        - Drop remaining rows with missing values in the province column.
        """
        self.data.loc[
            (self.data["province"].isna())
            & (self.data["region"] == "Région De Bruxelles-Capitale"),
            "province",
        ] = "Région De Bruxelles-Capitale"

        # Drop rows where 'province' is still missing
        self.data = self.data.dropna(subset=["province"])
